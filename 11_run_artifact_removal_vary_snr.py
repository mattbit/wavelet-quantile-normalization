...
from methods import (WaveletThresholding, OriginalSuBAR,
                     WaveletQuantileNormalization)
from tools import calculate_metrics, mask_to_intervals

datasets = ['data/03_denoise-net_emg_-1dB.h5',
            'data/03_denoise-net_eog_-1dB.h5']
...
for method_name, method in methods.items():
    if Path(f'results/vary_snr_metrics_{method_name}.csv').exists():
        continue
    ...
    for dataset_path in datasets:
            ...
            for snr in snrs:
                ...
                for i, record in tqdm(enumerate(dataset), total=len(dataset),
                    ...
                    _m = calculate_metrics(
                        restored, signal, reference, artifact_mask)
        ...
        pd.DataFrame(_metrics).set_index(['record']).to_csv(
            f'results/vary_snr_metrics_{method_name}.csv')

# %%

metrics = pd.concat([pd.read_csv(p)
                     for p in Path('results').glob('vary_snr_metrics_*.csv')])

metrics.to_csv('results/vary_snr_metrics.csv')
